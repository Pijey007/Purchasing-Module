...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        $item_sql = "INSERT INTO service_request_items 
            (request_id, item_code, description, unit_price, total_price) 
            VALUES (?, ?, ?, ?, ?)";
        ...
        foreach ($_POST['item_code'] as $i => $code) {
            ...
            $item_stmt->bind_param("issdd", $request_id, $code, $description, $unit_price, $total_price);
            $item_stmt->execute();
        }
...
<head>
    ...
    <link rel="stylesheet" href="myproject/css/service_request.css">
    <title>Purchasing System - Service Request Form</title>
</head>
<body>
    <div class="container">
        ...
        <form id="itemForm" method="POST">
            ...
            <button type="button" class="add-row" onclick="addRow()">+ Add Service</button>
    ...
    <script src="myproject/JS/service_request.js"></script>

    <?php if ($success): ?>
    <script>
        alert("Service request successfully submitted!");
        window.location.href = "service_request_form.php";
    </script>
    <?php endif; ?>
</body>
</html>